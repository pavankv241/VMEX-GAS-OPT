...
library BalancerOracle {
	...
	function calc_balancer_lp_price(
		...
		bool legacy
    		...
    		(uint fairResA, uint fairResB) =
    		  computeFairReserves(
				balances[0] * 1e18 / 10 ** (tokens[0].decimals()),
				balances[1] * 1e18 / 10 ** (tokens[1].decimals()),
				weights[0],
				weights[1],
    		    pxA,
    		    pxB
	...
	function get_balancer_variables(
		address bal_pool
	) 
	internal returns (
		IBalancer pool, 
